...
def execute_model1(candset, candset_l_key_attr, candset_r_key_attr,
    ...
    f = open('feature_costs', 'r')
    feat_cost = {}
    for l in f:
        feat_cost[l.strip().split(',')[0]] = float(l.strip().split(',')[1])*1000
    f.close()   
    ...
    for i in xrange(n):
        ...
        feat_index_cost.append(feat_cost[feature_attrs[i]])

    (neg_coeffs, pos_coeffs) = gen_ordering_by_coeffs(coeff, n, feat_index_cost) 
    pos_upper_bound = sum(map(lambda e: e[1], pos_coeffs))                                                                 
...
def gen_ordering_by_coeffs(coeff, n, feat_index_cost):
    coeffs_tuple = [(coeff[i]/feat_index_cost[i], coeff[i], i) for i in xrange(n)]
    neg_coeffs = []
    pos_coeffs = []
